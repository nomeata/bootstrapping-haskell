...
xmlParse :: String -> Document
dtdParse :: String -> Prolog

xmlParse = sanitycheck . papply document emptySTs . xmlLex
dtdParse = sanitycheck . papply prolog   emptySTs . xmlLex
...
either :: Parser SymTabs Token a -> Parser SymTabs Token b -> Parser SymTabs Token (Either a b)
either p q =
    ( p >>= return . Left) +++
    ( q >>= return . Right)
    ...
    ( do n <- pereference
         ...
         case tr of
           (Just (PEDefEntityValue ev)) ->
                      do pn <- posn
                         trace ("Def:\n  "++flattenEV ev++"\n") $
                           reparse (xmlReLex pn (flattenEV ev))
                         peRef p
           (Just (PEDefExternalID eid)) ->
                      do peRef p
...
checkmatch :: Name -> Name -> Parser SymTabs Token ()
checkmatch n m =
  if n == m then return ()
  else do p <- posn
          error ("Error at "++show p++": tag <"++n++"> terminated by </"++m++">")
