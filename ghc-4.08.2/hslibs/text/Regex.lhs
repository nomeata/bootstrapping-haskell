...
re_compile_pattern str single_line_mode insensitive
 = createPatBuffer insensitive	>>= \ (PatBuffer# pbuf) ->
   (if single_line_mode then	-- match a multi-line buffer
       _casm_ ``re_set_syntax(RE_PERL_SINGLELINE_SYNTAX);''
    else
       _casm_ ``re_set_syntax(RE_PERL_MULTILINE_SYNTAX);'') >>

   _casm_ ``  (int)re_compile_pattern((char *)%0,
   					(int)%1,
					(struct re_pattern_buffer *)%2);''
		(psToByteArray str) (lengthPS str) pbuf	>>= \ () ->
   --
...
re_match (PatBuffer# pbuf) str start reg
...
re_match2 (PatBuffer# pbuf) str1 str2 start stop reg
 = ((if reg then  -- record result of match in registers
      _casm_ ``%r = (struct re_registers *)malloc(sizeof(struct re_registers));''
     else
      _casm_ ``%r = (struct re_registers *)NULL;'')::IO Addr)	>>= \ regs ->
...
re_search (PatBuffer# pbuf) str start range reg
 = (if reg then  -- record result of match in registers
      _casm_ ``%r = (struct re_registers *)malloc(sizeof(struct re_registers));''
    else
...
re_search2 (PatBuffer# pbuf) str1 str2 start range stop reg
    ...
    trundleIO rs i acc len
     ...
     | otherwise	  
	  ...
	  if (start == (-1)) && (end == (-1)) then
	     trundleIO rs (i+1) ((1,1):acc) len
	  else
	     trundleIO rs (i+1) ((start,end):acc) len
\end{code}
