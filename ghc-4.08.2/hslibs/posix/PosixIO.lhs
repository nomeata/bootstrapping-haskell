...
import PrelHandle	( newHandle, getBMode__, getHandleFd, 
			  handleFinalizer, stdHandleFinalizer )
import IO
import Addr		( nullAddr )
import ForeignObj	( newForeignObj )
import CString		( freeze, allocChars, packString, unpackNBytesBA )
...
fdToHandle fd = do
	...
	fixIO $ \hdl -> do
	   ...
	   fo <- newForeignObj fo 
		 (if fd == stdInput || fd == stdOutput || fd == stdError
			then stdHandleFinalizer hdl
			else handleFinalizer hdl)
#endif
	   (bm, bf_size)  <- getBMode__ fo
           mkBuffer__ fo bf_size
	   hdl <- newHandle (Handle__ fo handle_t bm fd_str [])
	   return hdl
       else
         syserr "fdToHandle"
     else
       syserr "fdToHandle"
  where
   fd_str = "<file descriptor: " ++ show (fdToInt fd) ++ ">"
...
fdWrite fd str = do
    rc  <- _ccall_ write fd (packString str) (length str)
    if rc /= ((-1)::Int)
       then return rc
       else syserr "fdWrite"
...
bytes2ProcessIDAndLock :: MutableByteArray RealWorld Int -> IO (ProcessID, FileLock)
