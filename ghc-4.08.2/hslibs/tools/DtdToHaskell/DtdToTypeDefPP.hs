...
import Char (isLower, isUpper, toLower, toUpper)
...
data Name = Name String String	-- original name & mangled Haskell name
          deriving Eq
...
mkAttrDef (AttDef n StringType def) =
    []
mkAttrDef (AttDef n (TokenizedType t) def) =
    [] -- mkData [[String]] [] False (name n)
mkAttrDef (AttDef n (EnumeratedType (NotationType nt)) def) =
    error "NYI: attribute of EnumeratedType/NotationType"
mkAttrDef (AttDef n (EnumeratedType (Enumeration es)) def) =
    [EnumDef (name n) (map name es)]
        -- Default attribute values not handled

mkAttrField :: String -> AttDef -> (Name,StructType)
mkAttrField e (AttDef n StringType REQUIRED) = (name_f e n, String)
mkAttrField e (AttDef n StringType IMPLIED)  = (name_f e n, Maybe String)
mkAttrField e (AttDef n _ REQUIRED)          = (name_f e n, Defined (name n))
mkAttrField e (AttDef n _ IMPLIED)           = (name_f e n, Maybe (Defined (name n)))
mkAttrField e (AttDef n _ (Default atval f)) = (name_f e n, Defined (name n))
        -- Default attribute values not handled correctly
        ...
        parens (
          case st of
            ...
            (String)    -> text "definite fromText" <+> text "\"text\" \"" <>
                                                 ppXName n <> text "\"" <+> cvo
            (Defined m) -> text "definite fromElem" <+> text "\"<" <> ppXName m <> text ">\" \"" <>
                                                 ppXName n <> text "\"" <+> cvo
          )
  ...
  where
    toElem st v =
      case st of
        ...
        (Tuple ss)      -> text "toElem'" <+> v		-- not sure
        (OneOf ss)      -> text "toElem'" <+> v		-- not sure
...
mkToAttr n = text "toAttrFrTyp n" <+> ppHName n <+> text "=" <+>
             text "Just (n, str2attr \"" <> ppXName n <> text "\")"
...
mkFrFld tag (n,st) =
    ...
    ( case st of
        ...
        String         -> text "definiteA fromAttrToStr \"" <> ppXName tag <> text "\" \""
        _              -> text "definiteA fromAttrToTyp \"" <> ppXName tag <> text "\" \""
    ) <> ppXName n <> text "\" as"

mkToFld :: (Name,StructType) -> Doc
mkToFld (n,st) =
    ( case st of
        (Maybe String) -> text "maybeA toAttrFrStr \""
        (Maybe _)      -> text "maybeA toAttrFrTyp \""
        String         -> text "toAttrFrStr \""
        _              -> text "toAttrFrTyp \""
    ) <> ppXName n <> text "\"" <+> parens (ppHName n <+> text "v")
