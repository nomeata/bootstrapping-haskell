...
filterGT y h = unionList (collect h [])
...
filterGE y h = unionList (collect h [])
  where collect E hs = hs
        collect h@(L _ x a b) hs
          | x >= y = h : hs
...
partitionLT_GE y h = (h', unionList hs)
...
partitionLE_GT y h = (h', unionList hs)
  where (h', hs) = collect h []
...
partitionLT_GT y h = (h', unionList hs)
  where (h', hs) = collect h []
...
instance Ord a => CollX Heap a where
...
instance Ord a => OrdCollX Heap a where
...
instance Ord a => Coll Heap a where
...
instance Ord a => OrdColl Heap a where
