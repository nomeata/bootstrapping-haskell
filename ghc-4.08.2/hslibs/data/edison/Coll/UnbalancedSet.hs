...
import Collection ( CollX(..), OrdCollX(..), Coll(..), 
		    OrdColl(..), SetX(..), OrdSetX(..), OrdSet(..),
		    fromSeqWith_, insertWith_, insertSeqWith_,
		    unionl_, unionr_, unionWith_, unionSeqWith_,
		    intersectWith_,
                    toOrdList
		  )
...
instance Ord a => CollX Set a where
...
instance Ord a => OrdCollX Set a where
...
instance Ord a => Coll Set a where
...
instance Ord a => OrdColl Set a where
...
instance Ord a => SetX Set a where
  {intersect_ = intersect; difference_ = difference;
   subset_ = subset; subsetEq_ = subsetEq}

instance Ord a => C.Set Set a where
  {fromSeqWith_ = fromSeqWith; insertWith_ = insertWith; 
   insertSeqWith_ = insertSeqWith; unionl_ = unionl; unionr_ = unionr;
   unionWith_ = unionWith; unionSeqWith_ = unionSeqWith;
   intersectWith_ = intersectWith}

instance Ord a => OrdSetX Set a

instance Ord a => OrdSet Set a


instance Ord a => Eq (Set a) where
  xs == ys = toOrdList xs == toOrdList ys

--instance (Ord a, Show a) => Show (Set a) where
--  show xs = show (toOrdList xs)
