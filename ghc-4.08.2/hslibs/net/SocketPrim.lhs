...
--    recvmsg		-- :: Socket -> MsgFlags -> IO Message
    ...
    PortNumber(..),	 -- instance (Eq, Ord, Enum, Num, Real, Integral)
...
import PrelHandle   hiding ( mayBlock )
import PrelConc	    ( threadWaitRead, threadWaitWrite )
import ForeignObj   ( newForeignObj )
...
-- ToDo: need to protect socket with an MVar.  Protect whole
-- socket structure or just the SocketStatus?
\end{code}
...
instance Integral PortNumber where
    quotRem a b = let (c,d) = quotRem (ntohs a) (ntohs b) in
		  (mkPortNumber c, mkPortNumber d)
    toInteger a = toInteger (ntohs a)
    toInt a     = ntohs a
 ...
 if currentStatus /= NotConnected 
  then
   ...
   case (status::Int) of
     ...
     -6 -> do threadWaitWrite s
	      err <- getSocketOption sock SoError
	      if (err == 0)
		then writeIORef socketStatus Connected
		else do	cvtConnectSocketErr err isDomainSocket
		        constructErrorAndFail "connect"
     _  -> writeIORef socketStatus Connected

foreign import "cvtConnectSocketErr"
  cvtConnectSocketErr :: Int -> Int -> IO ()
\end{code}
...
accept sock@(MkSocket s family stype protocol status) = do
 ...
 if not okay
   then
     ...
     new_sock <- mayBlock s $ _ccall_ acceptSocket s ptr int_star
     case (new_sock::Int) of
	  -1 -> constructErrorAndFail "accept"
	  _  -> do a_sz <- stToIO (readIntArray int_star 0)
...
writeSocket (MkSocket s _family _stype _protocol status) xs = do
 currentStatus <- readIORef status
 nbytes <- mayBlock s $ _ccall_ writeDescriptor s (packString xs) (length xs)
 case (nbytes::Int) of
...
sendTo (MkSocket s _family _stype _protocol status) xs addr = do
 ...
 addr' <- packSockAddr addr
 let (_,sz) = boundsOfMutableByteArray addr'
 nbytes <- mayBlock s $ _ccall_ sendTo__ s (packString xs) (length xs) addr' sz
 case (nbytes::Int) of
...
readSocket (MkSocket s _family _stype _protocol status) nbytes = do
 ...
 rlen <- mayBlock s $ _ccall_ readDescriptor s ptr nbytes
 case (rlen::Int) of
   -1 -> constructErrorAndFail "readSocket"
   n  -> do barr <- stToIO (unsafeFreezeByteArray ptr)
	    let str = unpackNBytesBA barr n
            return (str, n)
...
recvFrom (MkSocket s _family _stype _protocol status) nbytes = do
 ...
 rlen   <- mayBlock s $ _ccall_ recvFrom__ s ptr nbytes ptr_addr
 case (rlen::Int) of
...
mayBlock :: Int -> IO Int -> IO Int
mayBlock fd act = do
   rc <- act
   case rc of
     -5 -> do  -- (possibly blocking) read
        threadWaitRead fd
	mayBlock fd act  -- input available, re-try
     -6 -> do  -- (possibly blocking) write
        threadWaitWrite fd
	mayBlock fd act  -- output possible
     _ -> do
        return rc
\end{code}
...
-- Alpha running OSF or a SPARC with SunOS, rather than Solaris.
       ...
       aix_TARGET_OS || freebsd2_TARGET_OS || freebsd_TARGET_OS || \
       netbsd_TARGET_OS || netbsd_elf_TARGET_OS || openbsd_TARGET_OS
data SocketType = 
	  Stream 
	| Datagram
	| Raw 
...
inet_addr :: String -> IO HostAddress
inet_addr ipstr = do
   had <- _ccall_ inet_addr (packString ipstr)
...
packSockAddr (SockAddrUnix path) = do
    ...
    _casm_ ``strcpy ((((struct sockaddr_un *)%0)->sun_path),%1);''    ptr 
	(packString path)
    return ptr
#endif
...
socketToHandle (MkSocket fd _ _ _ _) m = do
	...
	fixIO $ \hdl -> do
            fo <- newForeignObj fileobj (handleFinalizer hdl)
            mkBuffer__ fo 0  -- not buffered
            hdl <- newHandle (Handle__ fo htype NoBuffering socket_str [])
            return hdl
 where
