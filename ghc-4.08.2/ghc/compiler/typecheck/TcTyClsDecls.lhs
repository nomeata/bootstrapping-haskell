...
mk_cls_edges decl@(ClassDecl ctxt name _ _ _ _ _ _ _ _ _ _)
  = Just (decl, getUnique name, uniqSetToList (get_ctxt ctxt))
...
----------------------------------------------------
get_ctxt ctxt = unionManyUniqSets (map get_clas ctxt)
get_clas (HsPClass clas tys) = set_name clas `unionUniqSets` unionManyUniqSets (map get_ty tys)
get_clas _                   = emptyUniqSet
	-- These days, a context can contain types, not just
	-- type variables.  E.g. 
...
get_sigs sigs
  ...
  where 
    ...
    get_sig other		    = emptyUniqSet
	-- Fixity sigs, inline sigs
