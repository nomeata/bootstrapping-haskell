  ...
  = pushSrcLocRn src_loc $
	...
	fvs1 = case imp_exp of
		...
		FoExport   | isDyn	-> mkNameSet [makeStablePtr_NAME,
			   ...
			   | otherwise  -> mkNameSet [name', returnIO_NAME]
		_ -> emptyFVs
    in
...
checkConstraints doc forall_tyvars tau_vars ctxt ty
   = mapRn (checkPred doc forall_tyvars tau_vars ty) ctxt `thenRn` \ maybe_ctxt' ->
     returnRn (catMaybes maybe_ctxt')
	    -- Remove problem ones, to avoid duplicate error message.
	
checkPred doc forall_tyvars tau_vars ty p@(HsPClass clas tys)
  | ambiguous = failWithRn Nothing (ambigErr doc p ty)
  ...
  where
      ...
      ambiguous = -- All the universally-quantified tyvars in the constraint must appear in the tau ty
		  ...
		  any (\ct_var -> (ct_var `elem` forall_tyvars) && not (ct_var `elem` tau_vars)) ct_vars
...
checkPred doc forall_tyvars tau_vars ty p@(HsPIParam _ _)
  = returnRn (Just p)
