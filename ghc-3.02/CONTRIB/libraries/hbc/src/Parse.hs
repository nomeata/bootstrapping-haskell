...
data FailAt a
--TEMP:	= FailAt !Int [ErrMsg] a		 	-- token pos, list of acceptable tokens, rest of tokens
...
--TEMP:	| One b !Int a !(FailAt a)	-- parse succeeded with one parse
	| One b Int a (FailAt a)	-- parse succeeded with one parse
--TEMP:	| None !Bool !(FailAt a)		-- parse failed. The Bool indicates hard fail
